var makeLeaper = function(varX, varY, varZ) {
  var data = {
    "name": "leaper",
    "type": "enemy",
    "size": 0.5,
  ...
  data.act = function() {
    ...
    if (!data.jumpLock && data.hopTimer >= data.hopTimerMax) {
      data.velocity.x = data.speed * data.speedUp * data.direction.x;
      data.velocity.z = data.speed * data.speedUp * data.direction.z;
...
module.exports = function() { this.makeLeaper = makeLeaper; };
