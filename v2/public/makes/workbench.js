var makeWorkbench = function(varX, varY, varZ) {
  var data = {
    "name": "workbench",
    "type": "neutral",
    "size": 1.0,
  ...
  data.interact = function(sourceData) {
	...
	if (sourceData.carriedObject != null && objectGrabbed.name == 'wood') {
		makeLog(data.position.x, data.position.y, data.position.z + 2*BLOCKSIZE);
...
module.exports = function() { this.makeWorkbench = makeWorkbench; };
