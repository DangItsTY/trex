var makeSmellyzombie = function(varX, varY, varZ) {
  var data = {
    "name": "smellyzombie",
    "type": "enemy",
    "size": 0.5,
  ...
  data.act = function() {
    //  make cloud
    if (data.cloud == null) {
      makeCloud(data.position.x, data.position.y, data.position.z);
      data.cloud = objects[objects.length - 1];
      data.cloud.owner = data.uniqueIndex;
    }
...
module.exports = function() { this.makeSmellyzombie = makeSmellyzombie; };
