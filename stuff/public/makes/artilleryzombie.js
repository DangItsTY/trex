var makeArtilleryzombie = function(varX, varY, varZ) {
  var data = {
    "name": "artilleryzombie",
    "type": "enemy",
    "size": 0.5,
    "health": 1,
    "height": 11,
  ...
  data.act = function() {
    if (data.shootTimer >= data.shootTimerMax) {
      ...
      makeExploisonspit(ownerObject.position.x + (ownerObject.direction.x * offset), ownerObject.position.y, ownerObject.position.z + (ownerObject.direction.z * offset), ownerObject.direction.x, ownerObject.direction.z);
      data.shootTimer = 0;
...
module.exports = function() { this.makeArtilleryzombie = makeArtilleryzombie; };
