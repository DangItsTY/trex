var makeBubblezombie = function(varX, varY, varZ) {
  var data = {
    "name": "bubblezombie",
    "type": "enemy",
    "size": 0.5,
  ...
  data.act = function() {
    if (data.shootTimer >= data.shootTimerMax) {
      ...
      makeBubble(ownerObject.position.x + (ownerObject.direction.x * offset), ownerObject.position.y, ownerObject.position.z + (ownerObject.direction.z * offset), ownerObject.direction.x, ownerObject.direction.z);
      data.shootTimer = 0;
...
module.exports = function() { this.makeBubblezombie = makeBubblezombie; };
