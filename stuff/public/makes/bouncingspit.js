var makeBouncingspit = function(varX, varY, varZ, dirX, dirZ) {
  var data = {
    "name": "bouncingspit",
    "type": "enemy",
    "size": 0.25,
  ...
  data.act = function() {
    ...
    if (Math.floor(data.position.y) <= 0 && data.velocity.y <= 0) {
      data.velocity.y *= -1;
    } else if (
  ...
  data.collide = function(targetData) {
    if (data.alive && data.collision == "solid" && targetData.alive && targetData.type == "player" && typeof(targetData.onDamage) == 'function') {
      ...
      data.alive = false;
    } else if (data.alive && data.collision == "solid" && targetData.alive && targetData.type != "player") {
      data.velocity.x *= -1;
      data.velocity.y *= -1;
      data.velocity.z *= -1;
    }
...
module.exports = function() { this.makeBouncingspit = makeBouncingspit; };
